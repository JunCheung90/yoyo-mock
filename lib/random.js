...
var random = {
    ...
    normal_distribution: function (mean, std_dev) {
        if (std_dev < 0) {
            throw new Error("std_dev is incorrect");
        }

        return mean + (this.std_normal_distribution() * std_dev);
    },

    nd_random_in_range: function (config) {
        var r;
        var mean = config.mean || 0;
        var std = config.std || 0;
        var min = config.min || config.mean;
        var max = config.max || config.mean;
        if (std == 0 && (min != mean || max != mean)) {
            throw new Error("the max or min is incorrect");
        }
        do {
