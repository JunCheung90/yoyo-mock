...
var user = {
    ...
    contacts: function (contactsAmountConfig, contactsRepeatRateConfig, contactsSimilarRateConfig) {
        var sourceContacts = [];
        var targetContacts = [];
        var amountOfContact = Math.floor(Random.nd_random_in_range(contactsAmountConfig));
        if (amountOfContact / 2 == 1) { //若是奇数，先增加一个联系人
            targetContacts.push(Helpers.createYoYoUser());
        }
